...
@section('content')
    <div class="pattern_2">
                    ...
                    <div class="col-md-4 col-xl-3" data-cue="slideInUp">
                        <div class="item menu_item_grid">
                            <div class="item-img magnific-gallery" data-cue="slideInUp">
                                <img src="{{ url('uploads/product_images/'.$item->thumbnail) }}" alt="menu item">
                                <div class="content">
                                    <a href="{{ url('uploads/product_images/'.$item->thumbnail) }}" title="{{ $item->name }}"
                            ...
                            <div class="price_box">
                                <span class="new_price">#{{ $item->sales_price }}</span>
                                {{-- <span class="old_price">${{ $item->sales_price }}</span> --}}
                            </div>
                                <a href="{{ route('cart.product.add', ['page_id' => $page_id, 'id' => $item->id]) }}" class="btn_1">add to cart</a>
                        </div>
                    </div>
                    @endforeach
                @endif
        ...
        <div class="banner lazy">
            <p class="text-center"><a href="#0" class="btn_1 outline">Download Product (PDF)</a></p>
        </div>
    </div>
@endsection
